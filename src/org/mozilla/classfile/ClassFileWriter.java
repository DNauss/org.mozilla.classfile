...
public class ClassFileWriter {
    ...
    public void startMethod(String methodName, String type, short flags) {
        ...
        itsJumpFroms = new HashMap<Integer, Integer>();
    ...
    final class StackMapTable {
        ...
        private SuperBlock[] getSuperBlockDependencies() {
            ...
            for(int targetPC : itsJumpFroms.keySet()) {
            	Integer branch = itsJumpFroms.get(targetPC);
                int branchPC = branch ==  null ? -1 : branch.intValue();
...
*/
    ...
    private Map<Integer, Integer> itsJumpFroms = null;
    ...
    private List<ClassFileMethod> itsMethods = new ArrayList<ClassFileMethod>();
    private List<ClassFileField> itsFields = new ArrayList<ClassFileField>();
    private List<Short> itsInterfaces = new ArrayList<Short>();
...
// itsFixupTable[i] = (label_index << 32) | fixup_site
    ...
    private List<Object> itsVarDescriptors;
...
final class ConstantPool
{
    ...
    int addConstant(String k)
    {
        ...
        Integer index = itsStringConstHash.get(utf8Index);
        int theIndex = index == null ? -1 : index;
        if (theIndex == -1) {
    ...
    {
        Integer index = itsUtf8Hash.get(k);
        int theIndex = index == null ? -1 : index.intValue();
        if (theIndex == -1) {
            int strLen = k.length();
    ...
    {
        Integer index = itsClassHash.get(className);
        int theIndex = index == null ? -1 : index.intValue();
        if (theIndex == -1) {
            String slashed = className;
            if (className.indexOf('.') > 0) {
                slashed = ClassFileWriter.getSlashedForm(className);
                index = itsClassHash.get(slashed);
                theIndex = index == null ? -1 : index.intValue();
                if (theIndex != -1) {
    ...
    short addFieldRef(String className, String fieldName, String fieldType)
    {
        ...
        Integer index = itsFieldRefHash.get(ref);
        int theIndex = index == null ? -1 : index.intValue();
        if (theIndex == -1) {
                       ...
                       String methodType)
    {
        ...
        Integer index = itsMethodRefHash.get(ref);
        int theIndex = index == null ? -1 : index.intValue();
        if (theIndex == -1) {
    ...
    {
    	Byte type = itsPoolTypes.get(index);
        return type == null ? 0 : type.byteValue();
    }
    ...
    private Map<Integer, Integer> itsStringConstHash = new HashMap<Integer, Integer>();
    private Map<Object, Integer> itsUtf8Hash = new HashMap<Object, Integer>();
    private Map<Object, Integer> itsFieldRefHash = new HashMap<Object, Integer>();
    private Map<Object, Integer> itsMethodRefHash = new HashMap<Object, Integer>();
    private Map<Object, Integer> itsClassHash = new HashMap<Object, Integer>();

    private int itsTop;
    private int itsTopIndex;
    private Map<Integer, Object> itsConstantData = new HashMap<Integer, Object>();
    private Map<Integer, Byte> itsPoolTypes = new HashMap<Integer, Byte>();
